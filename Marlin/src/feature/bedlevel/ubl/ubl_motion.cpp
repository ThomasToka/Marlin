...
#include "../../../core/debug_out.h"
  ...
  bool __O2 unified_bed_leveling::line_to_destination_segmented(const feedRate_t scaled_fr_mm_s) {
    ...
    for (;;) {  // for each mesh cell encountered during the move
      ...
      xy_int8_t icell = {
        int8_t((raw.x - TERN(DYNAMIC_LEVELING, lcd_rts_settings.probe_margin_x, mesh_min.x)) * TERN(DYNAMIC_LEVELING, RECIPROCAL(get_mesh_x_dist()), RECIPROCAL(MESH_X_DIST))),
        int8_t((raw.y - TERN(DYNAMIC_LEVELING, lcd_rts_settings.probe_margin_y_front, mesh_min.y)) * TERN(DYNAMIC_LEVELING, RECIPROCAL(get_mesh_y_dist()), RECIPROCAL(MESH_Y_DIST)))
      };
