...
private:
  ...
  static void probe_entire_mesh(const xy_pos_t &near, const bool do_ubl_mesh_map, const bool stow_probe, const bool do_furthest OPTARG(DYNAMIC_LEVELING, uint8_t color_sp_offset)) __O0;
...
public:
  ...
  static bool cell_index_x_valid(const_float_t x) {
    return WITHIN(cell_index_x_raw(x), 0, (TERN(DYNAMIC_LEVELING, max_points.x - 1, GRID_MAX_CELLS_X)) - 1);
  }

  static bool cell_index_y_valid(const_float_t y) {
    return WITHIN(cell_index_y_raw(y), 0, (TERN(DYNAMIC_LEVELING, max_points.y - 1, GRID_MAX_CELLS_Y)) - 1);
  }

  static uint8_t cell_index_x(const_float_t x) {
    return constrain(cell_index_x_raw(x), 0, (TERN(DYNAMIC_LEVELING, max_points.x - 1, GRID_MAX_CELLS_X)) - 1);
  }

  static uint8_t cell_index_y(const_float_t y) {
    return constrain(cell_index_y_raw(y), 0, (TERN(DYNAMIC_LEVELING, max_points.y - 1, GRID_MAX_CELLS_Y)) - 1);
  }
   ...
   */
  static float z_correction_for_x_on_horizontal_mesh_line(const_float_t rx0, const uint8_t x1_i, const uint8_t yi OPTARG(DYNAMIC_LEVELING, const xy_uint8_t& _max_points)) {
    if (!WITHIN(x1_i, 0, (TERN(DYNAMIC_LEVELING, max_points.x, GRID_MAX_POINTS_X)) - 1) || !WITHIN(yi, 0, (TERN(DYNAMIC_LEVELING, max_points.y, GRID_MAX_POINTS_Y)) - 1)) {

      if (DEBUGGING(LEVELING)) {
        if (WITHIN(x1_i, 0, (TERN(DYNAMIC_LEVELING, max_points.x, GRID_MAX_POINTS_X)) - 1)) DEBUG_ECHOPGM("yi"); else DEBUG_ECHOPGM("x1_i");
    ...
    const float xratio = (rx0 - get_mesh_x(x1_i)) * TERN(DYNAMIC_LEVELING, RECIPROCAL(get_mesh_x_dist()), RECIPROCAL(MESH_X_DIST));
    const float z1 = z_values[x1_i][yi];

    return z1 + xratio * (z_values[_MIN(x1_i, (TERN(DYNAMIC_LEVELING, max_points.x, GRID_MAX_POINTS_X)) - 2) + 1][yi] - z1);  // Don't allow x1_i+1 to be past the end of the array
  ...
  //
  static float z_correction_for_y_on_vertical_mesh_line(const_float_t ry0, const uint8_t xi, const uint8_t y1_i OPTARG(DYNAMIC_LEVELING, const xy_uint8_t& _max_points)) {
    if (!WITHIN(xi, 0, (TERN(DYNAMIC_LEVELING, max_points.x, GRID_MAX_POINTS_X)) - 1) || !WITHIN(y1_i, 0, (TERN(DYNAMIC_LEVELING, max_points.y, GRID_MAX_POINTS_Y)) - 1)) {

      if (DEBUGGING(LEVELING)) {
        if (WITHIN(xi, 0, (TERN(DYNAMIC_LEVELING, max_points.x, GRID_MAX_POINTS_X)) - 1)) DEBUG_ECHOPGM("y1_i"); else DEBUG_ECHOPGM("xi");
    ...
    const float yratio = (ry0 - get_mesh_y(y1_i)) * TERN(DYNAMIC_LEVELING, RECIPROCAL(get_mesh_y_dist()), RECIPROCAL(MESH_Y_DIST));
    const float z1 = z_values[xi][y1_i];

    return z1 + yratio * (z_values[xi][_MIN(y1_i, (TERN(DYNAMIC_LEVELING, max_points.y, GRID_MAX_POINTS_Y)) - 2) + 1] - z1);  // Don't allow y1_i+1 to be past the end of the array
                                                                                        // If it is, it is clamped to the last element of the
                                                                                        // z_values[][] array and no correction is applied.
  }
  ...
  static float get_z_correction(const_float_t rx0, const_float_t ry0) {
    ...
    TERN(DYNAMIC_LEVELING, ,const) uint8_t mx = _MIN(cx, (TERN(DYNAMIC_LEVELING, max_points.x, GRID_MAX_POINTS_X)) - 2) + 1, my = _MIN(cy, (TERN(DYNAMIC_LEVELING, max_points.y, GRID_MAX_POINTS_Y)) - 2) + 1;
    TERN(DYNAMIC_LEVELING, ,const) float x0 = get_mesh_x(cx), x1 = get_mesh_x(cx + 1),
