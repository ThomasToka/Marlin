  ...
  void unified_bed_leveling::probe_entire_mesh(const xy_pos_t &nearby, const bool do_ubl_mesh_map, const bool stow_probe, const bool do_furthest
    ...
    probe.move_z_after_probing();
      ...
      motion.blocking_move_xy(
        constrain(nearby.x - probe.offset_xy.x, TERN(DYNAMIC_LEVELING, lcd_rts_settings.probe_margin_x, mesh_min.x), (TERN(DYNAMIC_LEVELING, (X_BED_SIZE - lcd_rts_settings.probe_margin_x), mesh_max.x))),
        constrain(nearby.y - probe.offset_xy.y, TERN(DYNAMIC_LEVELING, lcd_rts_settings.probe_margin_y_front, mesh_min.y), (TERN(DYNAMIC_LEVELING, (Y_BED_SIZE - lcd_rts_settings.probe_margin_y_back), mesh_max.y)))
      );
    #endif
    #if ENABLED(E3S1PRO_RTS)
      leveling_running = 0;
...
void set_message_with_feedback(FSTR_P const fstr) {
  ...
  float unified_bed_leveling::measure_business_card_thickness() {
    ...
    motion.blocking_move(
      xyz_pos_t({
        0.5f * ((TERN(DYNAMIC_LEVELING, (X_BED_SIZE - lcd_rts_settings.probe_margin_x), mesh_max.x)) - TERN(DYNAMIC_LEVELING, lcd_rts_settings.probe_margin_x, mesh_min.x)),
        0.5f * ((TERN(DYNAMIC_LEVELING, (Y_BED_SIZE - lcd_rts_settings.probe_margin_y_back), mesh_max.y)) - TERN(DYNAMIC_LEVELING, lcd_rts_settings.probe_margin_y_front, mesh_min.y)),
