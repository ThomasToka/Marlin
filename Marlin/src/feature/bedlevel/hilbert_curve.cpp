...
IF_DISABLED(DYNAMIC_LEVELING, constexpr) int8_t  to_fix(int8_t  v) { return v * 2; }
IF_DISABLED(DYNAMIC_LEVELING, constexpr) int8_t  to_int(int8_t  v) { return v / 2; }
IF_DISABLED(DYNAMIC_LEVELING, constexpr) uint8_t   log2(uint8_t n) { return (n > 1) ? 1 + log2(uint8_t(n >> 1)) : 0; }
IF_DISABLED(DYNAMIC_LEVELING, constexpr) uint8_t  order(uint8_t n) { return uint8_t(log2(uint8_t(n - 1))) + 1; }
...
static inline bool eval_candidate(int8_t x, int8_t y, hilbert_curve::callback_ptr func, void *data) {
  ...
  return x < (TERN(DYNAMIC_LEVELING, GRID_USED_POINTS_X, GRID_MAX_POINTS_X)) && y < (TERN(DYNAMIC_LEVELING, GRID_USED_POINTS_Y, GRID_MAX_POINTS_Y)) ? func(x, y, data) : false;
}
