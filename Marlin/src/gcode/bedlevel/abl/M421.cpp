...
void GcodeSuite::M421() {
  ...
  if (hasZ || hasQ) {
    if (WITHIN(ix, -1, TERN(DYNAMIC_LEVELING, GRID_USED_POINTS_X, GRID_MAX_POINTS_X) - 1) && WITHIN(iy, -1, TERN(DYNAMIC_LEVELING, GRID_USED_POINTS_Y, GRID_MAX_POINTS_Y) - 1)) {
      const float zval = parser.value_linear_units();
      uint8_t sx = ix >= 0 ? ix : 0, ex = ix >= 0 ? ix : TERN(DYNAMIC_LEVELING, GRID_USED_POINTS_X, GRID_MAX_POINTS_X) - 1;
      uint8_t sy = iy >= 0 ? iy : 0, ey = iy >= 0 ? iy : TERN(DYNAMIC_LEVELING, GRID_USED_POINTS_Y, GRID_MAX_POINTS_Y) - 1;
